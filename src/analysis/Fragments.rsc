...
public list[CFxy] matchFragments(list[CF] cl) =
	...
	[z | y <- [matchPair(cla, clb) | cla <- cl, clb <- cl], z <- y];
...
private list[CF] dropInvalidThreshold(list[tuple[location l, str s]] lst) =
  []
    when size(lst) == 0;
 
private default list[CF] dropInvalidThreshold(list[tuple[location l, str s]] lst) {
